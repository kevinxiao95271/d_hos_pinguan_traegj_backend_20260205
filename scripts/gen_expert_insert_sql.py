...
INST_USCC = {
    ...
    '树兰（安吉）医院':           '91330523MA28CUHJ9B',
    '萧山医院':                   '12330109754405020K',
    '浙江萧山医院':               '12330109754405020K',
    # EXT 机构
    '省医管中心':                 'EXT000000005',
...
EXTRA_USCC = {
    ...
    '浙江大学医学院附属第一医院':     '12330000470003222E',
    '浙江大学医学院附属第二医院':     '123300004700032309',
    '浙江大学医学院附属邵逸夫医院':   '12330000470003265U',
    '浙江大学医学院附属妇产科医院':   '12330000470003273N',
    '浙江大学医学院附属儿童医院':     '123300004700032571',
    '浙江大学医学院附属第四医院':     '12330782689145495P',
    '浙江大学医学院附属口腔医院':     '12330000470003281H',
    '浙江省人民医院':                 '12330000470051726F',
    '浙江省肿瘤医院':                 '123300004700517425',
    '浙江省中医院':                   '12330000470004129M',
    '浙江省儿童医院':                 '123300004700032571',
    '杭州市第一人民医院':             '12330100470116614F',
    '杭州市第三人民医院':             '12330100470116636Y',
    '杭州市第七人民医院':             '12330100470116673J',
    '杭州市儿童医院':                 '12330100470116643F',
    '杭州市肿瘤医院':                 '12330100470116639X',
    '杭州市中医院':                   '12330100470116681D',
    '杭州市妇产科医院':               '12330000470003273N',
    '树兰（杭州）医院':               '91330100321916626N',
    '宁波市第一医院':                 '12330200MB1P683255',
    '宁波大学附属第一医院':           '12330200MB1P683255',
    '宁波市第二医院':                 '123302004195298358',
    ...
    '温州医科大学附属第一医院':       '12330000470005922L',
    '温州医科大学附属第二医院':       '12330000470005930F',
    '温州医科大学附属眼视光医院':     '123303007743926187',
    '温州市中心医院':                 '12330300470525557C',
    '嘉兴市第一医院':                 '12330400470890351D',
    '嘉兴市第二医院':                 '12330400470890343J',
    '嘉兴市中医院':                   '12330400470890693Q',
    '嘉兴市中医医院':                 '12330400470890693Q',
    '湖州市中心医院':                 '123305004711717036',
    '湖州市第一人民医院':             '123305004711716660',
    '绍兴市人民医院':                 '12330600471323017X',
    '诸暨市人民医院':                 '12330681471481007R',
    '金华市中心医院':                 '12330700471540601G',
    '东阳市人民医院':                 '1233078347180163XH',
    '衢州市人民医院':                 '12330800471866062F',
    '舟山医院':                       '12330900472092185H',
    '浙江省台州医院':                 '12331000796490606H',
    '台州市中心医院':                 '12331000470460720G',
    '温岭市第一人民医院':             '123310817458062608',
    '丽水市中心医院':                 '12332500472260169F',
...
def bcrypt_hash(pwd: str) -> str:
    # jBCrypt (Java) 只识别 $2a$ 前缀，Python bcrypt 生成 $2b$，需替换
    h = bcrypt.hashpw(pwd.encode('utf-8'), bcrypt.gensalt(rounds=8)).decode('utf-8')
    return h.replace('$2b$', '$2a$')

# ── 读新专家文件（2026.3.16版本，仅用Sheet1）────────────────────────────────
...
EXCLUDE_PHONES = {
    '15867571130',  # 王泓权 - 已存在
    '15968083640',  # 潘翠萍 - 已存在
    '13588426741',  # 吕娜 - 今年不参与
}
for (name, inst), new_phone in PHONE_CORRECTIONS.items():
    df.loc[(df['姓名'] == name) & (df['单位'] == inst), '手机'] = new_phone

# ── 检查重复手机号（修正后不应有重复）────────────────────────────────────────
dup_phones = set(df[df.duplicated('手机', keep=False)]['手机'].tolist())
df_clean = df[~df['手机'].isin(dup_phones) & ~df['手机'].isin(EXCLUDE_PHONES)].copy()
print(f'total={len(df)}, dups={len(dup_phones)}, excluded={len(EXCLUDE_PHONES)}, clean={len(df_clean)}')
...
for _, row in df_clean.iterrows():
    ...
    title = row.get('职务', '') or ''
    title = str(title).strip() if title != 'nan' else ''
    ...
    title_val = title[:64] if title else ''
    title_sql = f"'{title_val}'" if title_val else 'NULL'

    sql = (
        f"INSERT INTO `user_accounts` (`name`, `phone`, `password`, `role`, `institution_id`, `title`, `expert_background`, `enabled`, `created_at`) "
        f"VALUES ("
        f"'{expert_name}', "
        f"'{phone}', "
        f"'{pwd_hash}', "
