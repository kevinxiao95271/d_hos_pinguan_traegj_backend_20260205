...
def test_reviewer_flow():
    ...
    competition_id = 23
    url = f"{BASE_URL}/api/reviews/my-tasks?competitionId={competition_id}"
    
    print(f"请求: GET {url}")
    print(f"参数: competitionId={competition_id}")
    ...
    print(f"✅ 获取成功，共 {len(tasks)} 个评审任务")
    ...
    if not tasks:
        ...
        for i, task in enumerate(tasks[:3], 1):
            ...
            print(f"     评审阶段: {task.get('reviewStage')}")
            print(f"     状态: {task.get('status')}")
        
        registration_id = tasks[0]["registrationId"]
