...
cur.execute("""
    UPDATE institutions SET city = '湖州市长兴路999号'
    WHERE id = 36310
""")
conn.commit()
print(f"更新成功，affected={cur.rowcount}")

cur.execute("SELECT id, name, code, uscc, region, city, level FROM institutions WHERE id = 36310")
r = cur.fetchone()
print(f"  id={r[0]}  {r[1]}  region={r[4]}  city={r[5]}  level={r[6]}")
conn.close()
